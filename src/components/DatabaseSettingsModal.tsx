  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-slate-900 border border-slate-800 rounded-3xl max-w-xl w-full p-6 space-y-5 shadow-2xl animate-in zoom-in-95 duration-200">
        ...
        <div className="p-3.5 rounded-2xl bg-slate-950 border border-slate-800/80 flex items-center justify-between gap-3 text-xs">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <div className="font-semibold text-slate-200">Tài khoản Google (OAuth)</div>
              <div className="text-[11px] text-slate-400">Dữ liệu hồ sơ: <strong>thuynga126@gmail.com</strong></div>
            </div>
          </div>
          ...
          <button
            ...
            onClick={async () => {
              ...
              const client = getSupabase();
              if (!client) {
                setStatusMessage({ text: 'Vui lòng nhập Supabase URL và Key trước khi đăng nhập!', type: 'error' });
                return;
              }
            ...
            className="px-3 py-1.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 text-xs font-semibold cursor-pointer transition-colors shrink-0"
          >
            Đăng nhập Google
          </button>
        </div>
